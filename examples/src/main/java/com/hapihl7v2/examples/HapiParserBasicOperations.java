...
public class HapiParserBasicOperations {
  ...
  public HapiParserBasicOperations() {
    String messageString =
        "MSH|^~\\&|SENDING_APPLICATION|SENDING_FACILITY|RECEIVING_APPLICATION|RECEIVING_FACILITY|20110614075841||ACK|1407511|P|2.4||||||\r\n"
        + "MSA|AA|1407511|Success||";

    // instantiate a PipeParser, which handles the "traditional or default
    // encoding"
    ...
    try {
      ...
      if (hl7Message instanceof ACK) {
        ...
        // note that I am using encode method at the end to convert it back to
        // string for display
        MSH mshSegmentMessageData = ackResponseMessage.getMSH();
        System.out.println("Message Type is " +
                           mshSegmentMessageData.getMessageType().encode());
        System.out.println(
            "Message Control Id is " +
            mshSegmentMessageData.getMessageControlID().encode());
        System.out.println(
            "Message Timestamp is " +
            mshSegmentMessageData.getDateTimeOfMessage().encode());
        System.out.println("Sending Facility is " +
                           mshSegmentMessageData.getSendingFacility().encode() +
                           "\n");
      ...
      // convert from default encoded message into XML format, and send it to
      // standard out for display
